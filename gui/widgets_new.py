...
def compute_simulation_ik(params):
    """
    Inverse Kinematics: Target pose → Pressures
    
    TODO: Implement your IK solver here
    
    This function should:
    1. Extract target position from params['target_x'], params['target_y'], params['target_z']
    2. Use IK parameters: params['ik_max_iter'], params['ik_tolerance'], params['ik_damping']
    3. Iteratively call compute_simulation_fk() with different pressures
    4. Return: (points, segments, info_text, solution_data)
    
    solution_data should be a dict containing:
        - 'pressures': {'A': value, 'B': value, 'C': value, 'D': value}
        - 'converged': True/False
        - 'iterations': number of iterations
        - 'final_error': distance to target
    """
    
    # YOUR IK IMPLEMENTATION GOES HERE
    # This is just a placeholder
    
    info_lines = []
    info_lines.append("⚠ IK solver not yet implemented!")
    info_lines.append("Please implement compute_simulation_ik() function\n")
    
    # Run FK with zero pressures as placeholder
    fk_params = params.copy()
    fk_params['channel_a'] = 0
    fk_params['channel_b'] = 0
    fk_params['channel_c'] = 0
    fk_params['channel_d'] = 0
    
    points, segments, fk_info = compute_simulation_fk(fk_params)
    info_lines.append(fk_info)
    
    solution_data = {
        'pressures': {'A': 0, 'B': 0, 'C': 0, 'D': 0},
        'converged': False,
        'iterations': 0,
        'final_error': 0
    }
